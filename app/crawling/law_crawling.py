...
url = "https://www.law.go.kr/DRF/lawSearch.do?OC=choi.tensor&target=prec&type=XML"
...
for i in trange(int(totalCnt / 20)):
    ...
    for node in items:
        ...
        판례상세링크 = "https://www.law.go.kr" + node.find('판례상세링크').text
        ...
        rows.append({'판례일련번호': 판례일련번호,
                     ...
                     '판례상세링크': 판례상세링크})
    page += 1
    url = "https://www.law.go.kr/DRF/lawSearch.do?OC=choi.tensor&target=prec&type=XML&page={}".format(page)
    response = urlopen(url).read()
    xtree = ET.fromstring(response)
